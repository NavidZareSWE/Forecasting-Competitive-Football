...
LINEUP_COLUMNS = {"match_id", "team_id", "team_name", "player_id", "player_name"}
...
def drop_log_rows(frame, mask, source, reason):
    ...
    record_id = dropped["event_id"] if "event_id" in dropped else dropped.index.astype(str)
    return pd.DataFrame({
        "source": source,
...
def clean_events(match_store):
    ...
    for match_id in match_store["match_id"]:
        ...
        events["event_time"] = pd.to_timedelta(events["timestamp"], errors="coerce")
        ...
        drop_frames.append(drop_log_rows(events, invalid, "events", "missing required event identity or timestamp"))
        events = events.loc[~invalid].copy()

        ordered = events.sort_values(["period", "event_time", "index"], kind="stable")
        assert ordered["event_id"].is_unique, f"Duplicate event_id in match {match_id}!"
        order_key = pd.MultiIndex.from_frame(ordered[["period", "event_time", "index"]])
        assert order_key.is_monotonic_increasing, f"Event ordering failed for match {match_id}!"
        ordering_rows.append({
            "scope": "events", "field": "period,timestamp,index", "affected_rows": int(
                (events["event_id"].to_numpy() != ordered["event_id"].to_numpy()).sum()
            ),
...
def clean_lineups(lineups):
    ...
    drop_log = drop_log_rows(lineups, invalid, "lineups", "missing match, team, or player identifier")
...
def canonical_map(observations, id_column, name_column, entity):
    ...
    observations["name_normalized"] = observations[name_column].str.strip().str.casefold()
    counts = observations.groupby([id_column, "name_normalized"], as_index=False).size()
    counts = counts.sort_values([id_column, "size", "name_normalized"], ascending=[True, False, True])
    ...
    variants = counts.groupby(id_column).size().rename("name_variants").reset_index()
...
def main():
    ...
    for path in [match_path, lineup_path]:
        if not path.exists():
            raise FileNotFoundError(f"Missing {path}. Run the Phase 1 stores before cleaning.")

    match_store = pd.read_csv(match_path, encoding="utf-8")
    lineups = pd.read_csv(lineup_path, encoding="utf-8")
    assert MATCH_COLUMNS <= set(match_store), "Rebuild match_store.csv to include stable team IDs."
    assert LINEUP_COLUMNS <= set(lineups), "lineups.csv is missing required identity columns."
    ...
    team_observations = pd.concat([
        ...
        clean_event_table[["team_id", "team"]].rename(columns={"team": "team_name"}),
    ], ignore_index=True)
    player_observations = pd.concat([
        clean_lineup_table[["player_id", "player_name"]],
        clean_event_table[["player_id", "player"]].rename(columns={"player": "player_name"}),
    ], ignore_index=True)

    team_map = canonical_map(team_observations, "team_id", "team_name", "team")
    player_map = canonical_map(player_observations, "player_id", "player_name", "player")
    drops = pd.concat([event_drops, lineup_drops], ignore_index=True)
    quality = quality_log(match_store, clean_event_table, clean_lineup_table, ordering_audit)

    PROCESSED_DIR.mkdir(parents=True, exist_ok=True)
    clean_event_table.to_csv(PROCESSED_DIR / "clean_events.csv", index=False, encoding="utf-8")
    clean_lineup_table.to_csv(PROCESSED_DIR / "clean_lineups.csv", index=False, encoding="utf-8")
    team_map.to_csv(PROCESSED_DIR / "team_identity_map.csv", index=False, encoding="utf-8")
    player_map.to_csv(PROCESSED_DIR / "player_identity_map.csv", index=False, encoding="utf-8")
    drops.to_csv(PROCESSED_DIR / "cleaning_drops.csv", index=False, encoding="utf-8")
    quality.to_csv(PROCESSED_DIR / "data_quality_log.csv", index=False, encoding="utf-8")

    print(f"Clean events: {len(clean_event_table)} | clean lineups: {len(clean_lineup_table)}")
    print(f"Canonical teams: {len(team_map)} | canonical players: {len(player_map)}")
    print(f"Dropped rows logged: {len(drops)} | quality findings logged: {len(quality)}")
