...
PREMATCH_GROUP_RULES = [
    ...
    ("schedule", lambda c: c.endswith(("rest_days", "played_prior"))
     and "h2h_" not in c),
    ("league", lambda c: c in {"competition_name", "era"}),
    ("head_to_head", lambda c: "h2h_" in c),
    ("elo", lambda c: c.startswith("elo_")),
    ("pi_ratings", lambda c: c.startswith("pi_")),
    ("xi_ratings", lambda c: "xi_" in c),
    ("squad_ratings", lambda c: "squad_" in c),
