...
A snapshot may combine the frozen pre-match vector with current-match events whose match clock is at or before *t*. Permitted in-play quantities include current score, cards/man advantage, and counts or rates over prior windows. Event records are ordered by `(period, index)`, not by timestamp: approximately twenty-six events across twenty-four matches carry corrupted `00:00:00` timestamps despite correct index positions, so the index is the only reliable within-period ordering key. The `minute` column is repaired with a running maximum before the cut, and the feature builder asserts on both sides that the selected prefix contains no event later than *t* and excludes none at or before *t*. Starting-lineup facts known by kick-off may be used, but card counts, substitutions, and other lineup facts observed after *t* may not. StatsBomb 360 data are excluded because the selected four league seasons have no 360 coverage.
...
`clean_store.py` orders every retained event by `(period, event_time, index)` and logs the number of rows whose position changed. The in-play feature builder deliberately re-sorts by `(period, index)` alone, because the timestamp is not trustworthy on every row (see section 1). Cleaning preserves each event's stable StatsBomb `team_id` and `player_id`. It builds canonical team and player maps from all observed names, so later feature code joins entities by ID rather than by spelling. Rows lacking an event identity, event index, period, timestamp, match identifier, team identifier, or player identifier are removed only when those fields are mandatory, and every removal is written to `cleaning_drops.csv` with a reason. Missing optional event fields (for example coordinates, xG, cards, or player identity on match-level events) and missing lineup coverage are retained and counted in `data_quality_log.csv` instead of being silently discarded.
